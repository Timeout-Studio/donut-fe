...
const parseUserResult = (userData: UserData | null) => {
  ...
  const parsedResults = userData.result.map(result => {
    let otherOrganisms: number[] = []
    try {
      const parsedJson = JSON.parse(result.other_organism_id)
      otherOrganisms = Object.values(parsedJson) as number[]
    } catch (error) {
...
const IndividualResultPage = ({ params }: PageProps) => {
  ...
  // 獲取用戶的主要生物和圖片
  const userMainOrganism = organisms.find(
    org => org.id.toString() === userData?.result[0]?.organism_id
  )
  const userAvatarImage = speciesImages.find(
    image => image.organismId === userMainOrganism?.id.toString()
  )
    ...
    <div className="min-h-screen bg-[#1E1E1E] text-white p-6">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <div className="bg-donut-accent rounded-full px-8 py-2">
          <h2 className="text-donut-text-white text-2xl font-bold">{userData?.level}</h2>
        </div>
            ...
            <div className="flex-grow flex items-center justify-center">
              <div className="text-5xl flex items-baseline gap-1 relative">
                <div className="font-bold">
                  {Math.floor((userData?.result[0]?.duration || 0) / 60)}
                </div>
                <div className="me-1 text-sm text-gray-400">分</div>
                <div className="font-bold">
                  {String((userData?.result[0]?.duration || 0) % 60).padStart(2, '0')}
                </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6 md:gap-8 min-w-[280px]">
          {organisms.map((organism) => {
            ...
            const currentResult = parsedResults.find(result => {
              return (
                result.organism_id === organism.id.toString() ||
                result.other_organism_id.includes(organism.id)
              )
            })

            // 獲取圖片
            const currentImage = speciesImages.find(
              image => image.organismId === organism.id.toString()
            )
            ...
            return (
              <div key={organism.id} className="flex flex-col items-center gap-2 min-w-[120px]">
                ...
                <span className={`text-sm text-center ${isLocked ? 'text-gray-500' : ''}`}>
                  {isLocked ? '???' : organism.name}
                </span>
              </div>
            );
          })}
